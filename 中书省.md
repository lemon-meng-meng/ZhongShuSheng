...
打开软件的一级界面我希望展示这些功能名称：授予二进制文件运行权限、在非“home/用户名称”这个主文件夹内相互移动文件和文件夹（注意，是非主文件夹，我主要希望把主文件夹内的文件或文件夹移动到/opt目录下）、删除文件和文件夹（主要目的是移除非“home/用户名称”目录下的文件夹或文件）、新建文件夹和文件、重命名文件和文件夹操作。然后可以点击应用内的一个按钮进入二级操作界面，在界面内实现这些操作。二级界面的流程主要应该是1.选择文件或文件夹2.执行相关操作3.输入密码4.完成操作。针对移动文件，应该是1.在以及界面点击“移动文件”按钮进入移动文件二级界面2.选择文件或文件夹3.选择目标目录4.输入密码5.完成移动。注意！！！我的这些操作主要是针对电脑“非主文件夹的目录”或者跨越主文件夹和非主文件夹之间的目录。这些操作都需要使用终端和输入密码，对新手来说极其麻烦。另外，上述我提到的文件和文件夹这个文件夹包含文件夹内的文件。
另外我想要在右键文件时直接进行操作，解释：
对文件进行右键会有一个一级选项“使用中书省操作”，在这个一级选项下出现以下二级选项并执行（首段文字是该二级选项的名称）
1.授予运行权限-如不支持，提示非可运行文件-如支持，授予运行权限
2.移动到-弹出目标文件夹选择页面-输入密码确认（因为目标文件夹是“opt”这样不允许直接写入的文件夹）
3.删除-提示删除非“home/用户名称”文件夹或文件的风险-输入密码确认
4.新建文件夹-名称输入-输入密码确认
5.重命名-输入重命名的名字-输入密码确认

请完成中书省这个应用的开发，“/home/lemon/应用/应用文件/Opencode/中书省”目录内是应用的雏形，所有开发产生的依赖文件都为我放在“/home/lemon/应用/应用文件/Opencode/中书省”目录内，并帮我完成Deb、AppImage的打包。应用界面现做一个简单点的，后续我们在慢慢优化，目录按照你的意愿随意整理，尽可能匹配科学的开发目录，方便我后续上传github
